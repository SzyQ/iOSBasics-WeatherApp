...
class NetworkWeatherRepository: WeatherRepository {
    ...
    func getWeatherInfo(latitude: CLLocationDegrees, longitude: CLLocationDegrees, completion: @escaping (WeatherInfo?, Error?) -> Void) {
        ...
        let parameters = ["lat" : String(latitude), "lon" : String(longitude), "appid" : Build.WEATHER_API_KEY]
...
fileprivate extension NSDictionary {
    ...
    func toWeatherInfo() -> WeatherInfo {
        ...
        return WeatherInfo(temperature: temp.intValue, pressure: Int(pressure), minTemperature: tempMin.intValue, maxTemperature: tempMax.intValue, imageUrl: iconURL, weatherDescription: description,originalJson: self.description)
    }
}
